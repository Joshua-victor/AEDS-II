...
public class Main {
    ...
    public static int compare(Pokemon j, Pokemon i) {
        int resp = j.getTypes().get(0).compareTo(i.getTypes().get(0));

        return resp;
    }
    public static void insercao(int len) {
        for (int i = 1; i < len; i++) {
            Pokemon tmp = array[i];
            int j = i - 1;
            if (array[j].getName().compareTo(tmp.getName()) > 0) {
                tp.addComp(1);
                while ((j >= 0) && (array[j].getTypes().get(0).compareTo(tmp.getTypes().get(0)) == 0) && array[j].getName().compareTo(tmp.getName()) > 0) {
                    tp.addComp(1);
                    array[j + 1] = array[j];
                    tp.addMov(1);
                    j--;
    ...
    public static void mergeSort(int len)
    {
        mergeSort(0, len -1);
    }
    ...
    public static void merge(int left, int mid, int right) {
        ...
        Pokemon[] auxLeft = new Pokemon[n1];
        Pokemon[] auxRight = new Pokemon[n2];

        for (int i = 0; i < n1; i++) {
            auxLeft[i] = array[left + i].myClone();
            tp.addMov(1);
        }

        for (int i = 0; i < n2; i++) {
            auxRight[i] = array[mid + i + 1].myClone();
            tp.addMov(1);
        }
        ...
        while (i < n1 && j < n2) {
            if (compare(auxLeft[i], auxRight[j]) < 0) {
                array[l] = auxLeft[i].myClone();
                i++;
                tp.addMov(1);
            } else {
                array[l] = auxRight[j].myClone();
                j++;
                tp.addMov(1);
            }
    ...
    public static void main(String args[]) {
        ...
        tp.ordenacaoFile("789339_mergesort.txt");

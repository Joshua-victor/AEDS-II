...
public class Main 
{
    ...
    public static int getMaior()
    {
        ...
        for(int i = 0; i < N; i++)
        {
            if(maior < array[i].getCaptureRate())
            {
    ...
    public static void insercao(int len)
    {
        for(int i = 1; i < len; i++){
            ...
            {
                tp.addComp(1);
                while((j >= 0) && (array[j].getCaptureRate() == tmp.getCaptureRate()) && array[j].getName().compareTo(tmp.getName()) > 0){
                    tp.addComp(1);
                    array[j + 1] = array[j];
                    tp.addMov(1);
                    j--;
    ...
    public static Pokemon[] coutingSort(int len){
        int maior = getMaior();

        int[] count = new int[maior + 1];

        Pokemon[] ordenado = new Pokemon[len];
        ...
        {
            ordenado[count[array[i].getCaptureRate()] - 1] = array[i];
            count[array[i].getCaptureRate()]--;
            tp.addMov(1);
        }
    ...
    public static void main(String args[])
    {
        ...
        tp.ordenacaoFile("789339_countingsort.txt");

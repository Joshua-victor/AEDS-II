...
void swap(Pokemon *array, int i, int j) {
    Pokemon tmp = array[i];
    array[i] = array[j];
    array[j] = tmp;
...
void constroi(Pokemon *pokemons, int tamHeap, int i) {
    while (i > 1) {
        int pai = i / 2;
        comparacoes += 2;

        if (pokemons[i].height > pokemons[pai].height || 
           (pokemons[i].height == pokemons[pai].height &&
            strcmp(pokemons[i].name, pokemons[pai].name) > 0)) {
            swap(pokemons, i, pai);
            i = pai;
        } else {
            break;
...
int getMaiorFilho(Pokemon *pokemons, int i, int tamHeap) {
    int filhoEsq = 2 * i;
    int filhoDir = 2 * i + 1;

    if (filhoDir > tamHeap) return filhoEsq;

    if (pokemons[filhoEsq].height > pokemons[filhoDir].height || 
       (pokemons[filhoEsq].height == pokemons[filhoDir].height &&
        strcmp(pokemons[filhoEsq].name, pokemons[filhoDir].name) > 0)) {
        return filhoEsq;
    } else {
        return filhoDir;
    }
...
void reconstroi(Pokemon *pokemons, int tamHeap) {
    int i = 1;
    while (i <= (tamHeap / 2)) {
        int filho = getMaiorFilho(pokemons, i, tamHeap);
        comparacoes += 3;

        if (pokemons[i].height < pokemons[filho].height || 
           (pokemons[i].height == pokemons[filho].height &&
            strcmp(pokemons[i].name, pokemons[filho].name) < 0)) {
            swap(pokemons, i, filho);
            i = filho;
        } else {
            break;
        }
...
void heapsortParcial(Pokemon *array, int n) {
...
int main()
{
    ...
    FILE *fw = fopen("789339_heapsortParcial.txt", "w");
    if (fw != NULL)
    {

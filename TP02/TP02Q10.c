...
void quickSort(int esq, int dir) {
    int i = esq, j = dir;
    Pokemon pivo = array[(esq + dir) / 2]; // Escolhe o elemento do meio como pivô
    ...
    while (i <= j) {
        ...
        if (i <= j) {
            swap(i, j); // Troca os elementos
            i++;
            j--;
        }
    }

    if (esq < j) quickSort(esq, j); // Ordena recursivamente à esquerda do pivô
    if (i < dir) quickSort(i, dir); // Ordena recursivamente à direita do pivô
}
...
int main() {
    ...
    quickSort(0, len - 1); // Substitui o shellSort pelo quickSort
    clock_t final = clock();
    ...
    FILE *fw = fopen("789339_quicksort.txt", "w");
    if (fw != NULL) {

...
int compare(Pokemon x, Pokemon y)
{
    ...
    comparacoes++;
    if (strcmp(x.abilities, y.abilities) > 0)
    {
        is = 1;
    }
    else if (strcmp(x.abilities, y.abilities) < 0)
    {
        is = -1;
    }
...
void bolha(int n) {
    ...
    for (i = 0; i < n - 1; i++) {
        for (j = 0; j < n - 1 - i; j++) {
            if (compare(array[j], array[j + 1]) > 0) { // Correção na comparação
                swap(j, j + 1); // Realiza a troca correta
            }
        }
    }
}
...
int main() {
    ...
    FILE *fw = fopen("789339_radixsort.txt", "w");
    if (fw != NULL) {

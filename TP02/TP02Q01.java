...
class PokemonSearch {
	// FunC'C#o estC!tica que busca um PokC)mon pelo ID
...
public class Main {
	public static void main(String[] args) {
		...
		List<Pokemon> pokemons = ReadCsv.readAllFile("/tmp/pokemon.csv"); // usado para fazer uma lista do csv todo


		List<Pokemon> usados = new ArrayList<>(); // para criar uma lista dos id's que vamos usar


		while (!(input_string = scanner.nextLine()).equals("FIM")) {  // usado para procurar no csv achar o id desejado  ou identificar se a palavra for FIM
			input_int = Integer.parseInt(input_string);
			usados.add(PokemonSearch.searchPokemonId(pokemons, input_int));
		}

		for (Pokemon p : usados) System.out.println(p);
